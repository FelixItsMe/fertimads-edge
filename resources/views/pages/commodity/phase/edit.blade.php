    ...
    <x-slot name="header">
        <h2 class="leading-tight">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item breadcrumb-active">{{ __('Edit Fase') }}</li>
            </ol>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('commodity.phase.update', $commodity->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @if ($errors)
                        {{ $errors->first() }}
                    ...
                    <div class="p-4 flex flex-col gap-2 gap-y-4">
                        @foreach ($commodity->commodityPhases as $commodityPhase)
                                ...
                                <div class="flex flex-col border-b-2 border-white">
                                    <div class="w-full">
                                        <x-input-label for="phase-{{ $loop->iteration }}"
                                            ...
                                            {{ __('Fase') }} {{ $loop->iteration }}: {{ $commodityPhase->phase->getLabelText() }}
                                        </x-input-label>
                                    </div>
                                </div>
                                <div class="grid grid-flow-row grid-cols-1 sm:grid-cols-3 gap-2">
                                    ...
                                    <div class="w-full">
                                        ...
                                        <x-text-input id="kc-{{ $loop->iteration }}" class="block mt-1 w-full rounded-xl" type="number"
                                            min="0" step=".01"
                                            name="phase[{{ $commodityPhase->phase->value }}][kc]" :value="$commodityPhase->kc" required autocomplete="kc" />
