    ...
    <x-slot name="header">
        <h2 class="leading-tight">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item breadcrumb-active">{{ __('Edit Komoditi') }}</li>
            </ol>
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('commodity.update', $commodity->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 flex flex-col gap-2">
                        <div class="grid sm:grid-flow-row sm:grid-cols-3 gap-2">
                            <div class="w-full">
                                <x-input-label for="name">{{ __('Gambar') }}</x-input-label>
                                <img src="{{ asset($commodity->image ?? 'images/default/default-image.jpg') }}" alt="Preview Image"
                        ...
                        <div class="flex flex-col">
                            <div class="w-full">
                                ...
                                <x-text-input id="name" class="block mt-1 w-full rounded-xl" type="text"
                                    name="name" :value="$commodity->name" required autofocus autocomplete="name" />
                        ...
                        <div class="flex flex-col">
                            <div class="w-full">
                                ...
                                <x-textarea id="description" class="block mt-1 w-full rounded-xl" name="description" required
                                    autocomplete="description">{{ $commodity->description }}</x-textarea>
                                <x-input-error :messages="$errors->get('description')" class="mt-2" />
                            </div>
                        </div>
                        <div class="flex flex-col">
                            <div class="w-full flex sm:justify-end">
                                <x-primary-button class="max-sm:w-full">
                                    {{ __('Simpan') }}
                                </x-primary-button>
                            </div>
                        </div>
